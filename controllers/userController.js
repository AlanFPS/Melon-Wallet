  ...
  register: async (req, res) => {
    ...
    try {
      ...
      if (user) {
        ...
        req.flash(
          'warning_msg',
          'A user with this email already exists. Choose a different address or login.'
        )
    ...
    try {
      if (file) {
        ...
        if (acceptedType.indexOf(fileType) === -1) {
          req.flash(
            'error_msg',
            'This type of image is not accepted. Please upload image with .png, .jpg, or .jpeg extension only. '
          )
